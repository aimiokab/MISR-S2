...
from models.misr_module import RRDBHighresNet
from utils.hparams import hparams
from tasks.trainer import Trainer
import json


class RRDBhighresnet(Trainer):
    def build_model(self):
        hidden_size = hparams['hidden_size']
        with open("./tasks/config_rrdb_misr.json", "r") as read_file:
            self.config = json.load(read_file)
        self.model = RRDBHighresNet(self.config["network"], 3, 3, hidden_size, hparams['num_block'], hidden_size // 2)
        self.global_step = 0
        return self.model
    ...
    def sample_and_test(self, sample):
        ...
        img_sr = self.model(img_lr, alphas, self.config)
