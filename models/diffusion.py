...
class GaussianDiffusion(nn.Module):
    ...
    def forward(self, img_hr, img_lr, img_lr_up, t=None, dates=None, config=None, alphas=None, *args, **kwargs):
        x = img_hr
        ...
        if hparams['use_rrdb']:
            ...
            self.rrdb.eval()
            with torch.no_grad():
                rrdb_out, cond = self.rrdb(img_lr, dates, config, True)
        elif hparams['use_highresnet']:
            self.rrdb.eval()
            with torch.no_grad():
                rrdb_out, cond = self.rrdb(img_lr, alphas, True)
        else:
            rrdb_out = img_lr_up
            cond = [img_lr]
    ...
    @torch.no_grad()
    def sample(self, img_lr, img_lr_up, shape, save_intermediate=False, dates=None, config=None, alphas = None):
