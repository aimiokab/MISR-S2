...
from dataset import BreizhSRDataset


def preprocess_and_save(
    path_to_dataset: str, split: str = "train", max_s2_images: int = 1
):
    ...
    output_folder = os.path.join(
        path_to_dataset, "preprocessed", sr_type, f"dataset_{split}"
    )
    ...
    dataset = BreizhSRDataset(path_to_dataset, split=split, max_s2_images=max_s2_images)
...
def preprocess_dataset(path_to_dataset, max_s2_images=1):
    ...
    df_train = df_train_sisr.join(
        df_train_misr[["path_lr_misr", "alphas", "dates_encoding"]]
    )
    df_test = df_test_sisr.join(
        df_test_misr[["path_lr_misr", "alphas", "dates_encoding"]]
    )
